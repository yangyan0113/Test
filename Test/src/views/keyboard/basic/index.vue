  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="练习类别" prop="seriesBig">
        <el-select v-model="queryParams.seriesBig" placeholder="请选择练习类别">
          <el-option
    ...
    <el-table v-loading="loading" :data="keyboardList">
      <el-table-column label="主键编号" v-if="false" align="center" prop="keyboardId"/> 
      ...
      <el-table-column label="标题" align="center" :show-overflow-tooltip="true">
       ...
       <!-- <el-table-column label="练习类别" align="center" prop="seriesBig"/>  -->
       <el-table-column label="练习类别" align="center" prop="bigDesc" />  
       <!-- <el-table-column label="文章类型" align="center" prop="smallDesc"/>  -->
       <el-table-column label="文章类型" align="center" prop="seriesSmall">  
        <template slot-scope="scope"> 
          <el-tag v-if="scope.row.seriesSmall == 1">学习</el-tag>
          <el-tag type="success" v-else-if="scope.row.seriesSmall == 2">练习</el-tag>
          <el-tag type="warning" v-else-if="scope.row.seriesSmall == 3">游戏</el-tag>
        </template> 
       </el-table-column> 
      ...
      <!--  
      <el-table-column label="类别" align="center" prop="seriesBig" width="70">
        <template slot-scope="scope">
          <dict-tag :options="bigCodeOptions" :value="scope.row.seriesBig"/>
        </template>
      </el-table-column> -->
    </el-table>
...
import { listCeKeyboardLog } from '@/api/exercise/ceKeyboardLog.js'
...
export default {
  ...
  methods: {
    ...
    showKeyData(row) {
      ...
      if (row.seriesSmall == '1')
        //学习
        this.$router.push({ path: '/study', query: { keyboardId } })
      else if (row.seriesSmall == '2')
        //练习
        this.$router.push({ path: '/practice', query: { keyboardId } })
      else if (row.seriesSmall == '3')
        //游戏
        this.$router.push({ path: '/game', query: { keyboardId } })
    },
  },
}
</script>
